...
在slurm集群中搭建Cromwell引擎。在进行多任务投递后，运行一段时间后会出现ssh断连的情况，同时cromwell后台也会中断。由于我的cromwell是在后台运行，因此排除是由于ssh超时导致的中断。一番搜索，确认原因大概是因为并行任务过多导致缓存溢出，使cromwell中断并且令ssh断开。

解决方法是调整配置文件，增大单个线程的需用的最低内存数（副作用是会导致排队任务多），由于cromwell配置文件中设置的内存数最终会导入到sbatch的--mem-per-cpu参数中，这个参数是设定单个线程需保留的**最低**内存数，当我们一次性提交多个作业，程序占用的内存是逐步升高的，因此提交时的内存余量较多会使调度程序认为有余量可加入作业。随着计算内存占用升高，最终会导致内存溢出触发oom使程序自动kill掉。
